...
import { backupFilename, createBackup, describeRestore, describeStored, readBackup } from '../lib/backup';
export function Shell(){
 ...
 // The path a navigation is waiting to announce, until that page's own meta
 // catches up (routes are lazily loaded, so this can lag the URL change).
 ...
 // The announcement is written into the live region imperatively, once per
 // navigation, and then left there rather than driven straight off render
 // state: a value that goes text -> '' -> text within one commit can come and
 // go faster than a screen reader (or a polling assertion) ever observes it.
 // announcedPath guards against writing it twice for the same navigation —
 // which matters because currentPage also updates for reasons that are not a
 // navigation at all, such as renaming the trip on the page already showing,
 // and that must not read out "page loaded" for a page nobody just arrived at.
 const announcedPath=useRef<string|null>(null);
 useEffect(()=>{
  if(pendingPath===null||currentPage.path!==pendingPath||announcedPath.current===pendingPath)return;
  announcedPath.current=pendingPath;
  if(announcerRef.current)announcerRef.current.textContent=`${currentPage.title}, page loaded`;
 },[pendingPath,currentPage]);
 const stored=saved.length+trips.length+compare.length;
